...
if (isset($_POST['editExam'])) {
    if (!(empty($_POST['lesson_id']) || empty($_POST['student_id']) || empty($_POST['exam_score']))) {
        ...
        if ($exam_score >= 0 && $exam_score <= 100) {
            $exam->editExam($_POST['id'], $_POST['student_id'], $_POST['lesson_id'], $_POST['exam_score']);
            $item = $exam->getExamById($_POST['id']);
...
<div class="content-wrapper">
    <div class="card-header">
        <h3 class="card-title">Sınav Güncelleme Sayfası</h3>
    </div>
        ...
        <form class="form" action="" method="post">
            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <input type="number" step="0.01" class="form-control" name="exam_score" value=<?php echo $item->exam_score ?>>
                </div>
            </div>
            <div class="card-footer">
                <button type="submit" name="editExam" class="btn btn-primary">Kaydet</button>
            </div>
        </form>
</div>
</div>
