...
if (isset($_POST['editStudent'])) {
    if (!(empty($_POST['id']) || empty($_POST['name']) || empty($_POST['surname']) || empty($_POST['username']) || empty($_POST['password']))) {
        if (!($database->getByUsernameForEdit($_POST['username'], $_POST['id']))) {
            $student->editStudent($_POST['id'], $_POST['name'], $_POST['surname'], $_POST['username'], $_POST['password']);
            $person = $student->getStudentById($_POST['id']);
...
<div class="content-wrapper">
    <div class="card-header">
        <h3 class="card-title">Öğrenci Güncelleme Sayfası</h3>
    </div>
    <section class="content">
        <form class="form" action="" method="post">
            ...
            <div class="card-footer">
                <button type="submit" name="editStudent" class="btn btn-primary">Güncelle</button>
            </div>
        </form>
</div>
</div>
